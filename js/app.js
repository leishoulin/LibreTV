...
// 全局变量
let selectedAPIs = []; // 默认选中资源
let customAPIs = []; // 存储自定义API列表
...
document.addEventListener('DOMContentLoaded', async function () {
    customAPIs = await fetchConfig("customApi", []);
    if (customAPIs.length === 0) {
        customAPIs = [];
    }
    // console.log("customAPIs:", customAPIs);

    selectedAPIs = await fetchConfig("selectedAPIs", []);
    if (selectedAPIs.length === 0) {
        selectedAPIs = ["tyyszy", "bfzy", "dyttzy", "ruyi"];
        await saveConfig("selectedAPIs", selectedAPIs);
    }
...
// 读取配置
async function fetchConfig(key, defaultValue = {}) {
...
function initAPICheckboxes() {
    ...
    Object.keys(API_SITES).forEach(apiKey => {
        ...
        // const checked = api["checked"] || false;
...
function updateCustomApi(index) {
    ...
    // todo
    saveConfig("customApi", customAPIs);
...
async function showAddCustomApiForm() {
    ...
    // let list = [{ "name": "飘零", "url": "https://p2100.net/api.php/provide/vod/", "detail": "", "isAdult": false }];
    // saveConfig("customApi",list);
    // console.log(fetchConfig('customApi'));
    // console.log(dotenv);
    // let res = await fetchConfig("customApi");
    // console.log(res);
    const form = document.getElementById('addCustomApiForm');
    if (form) {
        form.classList.remove('hidden');
...
async function addCustomApi() {
    ...
    // 保存 detail 字段
    customAPIs.push({ name, url, detail, isAdult, checked: true });
    await saveConfig("customApi", customAPIs);
    // localStorage.setItem('customAPIs', JSON.stringify(customAPIs));
...
// 移除自定义API
async function removeCustomApi(index) {
    if (index < 0 || index >= customAPIs.length) return;
    ...
    console.log("selectedAPIs1", selectedAPIs);
    ...
    console.log("selectedAPIs2", selectedAPIs);
    // localStorage.setItem('selectedAPIs', JSON.stringify(selectedAPIs));
    await saveConfig("selectedAPIs", selectedAPIs);
